...
const expectedNonRecursiveProduction = normalizeLineEndings(
  ...
  xfs.readFileSync(
    ppath.join(
      __dirname as PortablePath,
      "fixtures/expected/disclaimerProduction.txt" as PortablePath
    ),
    "utf8"
  )
);
...
const expectedRecursiveProduction = normalizeLineEndings(
  ...
  xfs.readFileSync(
    ppath.join(
      __dirname as PortablePath,
      "fixtures/expected/disclaimerRecursiveProduction.txt" as PortablePath
    ),
    "utf8"
  )
);
...
describe.each(["pnp", "node-modules"])(
  ...
  (linker) => {
    ...
    it("should generate disclaimer recursively for production", () => {
      const stdout = execSync(
        "yarn licenses generate-disclaimer --recursive --production",
        {
          cwd,
        }
      ).toString();
